 ...
 * Description:基于处理时间的定时器代码实现
 */
public class Flink09_TimeService_ProcessingTime {
    public static void main(String[] args) throws Exception {
        ...
        // 3.注册基于处理时间的定时器
        SingleOutputStreamOperator<String> result = keyedStream.process(new KeyedProcessFunction<Tuple
                , WaterSensor, String>() {
            @Override
            public void processElement(WaterSensor value, Context ctx, Collector<String> out) throws Exception {
                // 注册一个5秒后的定时器
                System.out.println("当前时间：" + ctx.timerService().currentProcessingTime()/1000);
                System.out.println("注册一个定时器，将在" + (ctx.timerService().currentProcessingTime() + 5000)/1000 + "时触发");
